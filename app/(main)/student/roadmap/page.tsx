...
const getStatusInfo = (
  status: string,
): { color: "default" | "primary" | "warning" | "success"; text: string } => {
  switch (status) {
    case "in_progress":
...
export default function StudentRoadmap() {
  ...
  const loadRoadmap = async () => {
    try {
      const data = await api.get<any[]>("/api/roadmap");
      const normalized = (data || []).map((block: any) => ({
        id: block.ID || block.id,
        title: block.Title || block.title,
        description: block.Description || block.description,
        progress: {
          status:
            block.progress?.status || block.Progress?.status || "not_started",
          percent: block.progress?.percent || block.Progress?.percent || 0,
        },
        materials: (block.Materials || block.materials || []).map(
          (mat: any) => ({
            id: mat.ID || mat.id,
            title: mat.Title || mat.title,
            description: mat.Description || mat.description,
            type: (mat.Type || mat.type || "").toLowerCase(),
            url: mat.URL || mat.url,
            is_required: mat.IsRequired || mat.is_required,
            viewed: mat.Viewed || mat.viewed || false,
          }),
        ),
      }));
  ...
  if (blocks.length === 0)
    return <div className="p-8 text-center text-text-muted">Нет блоков</div>;
      ...
      <Accordion variant="splitted" className="px-0 gap-4">
        {blocks.map((block) => {
          const statusInfo = getStatusInfo(
            block.progress?.status || "not_started",
          );
              ...
              title={
                <div className="flex flex-wrap justify-between items-center w-full pr-4 gap-2">
                  <div className="flex flex-col text-left">
                    <span className="text-base font-semibold text-text-main font-mono">
                      {block.title}
                    </span>
                    {block.description && (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 py-4 border-t border-border-subtle mt-2">
                {COLUMNS.map((col) => {
                  ...
                  return (
                    <div
                      key={col.type}
                      className="flex flex-col bg-canvas/40 rounded-lg p-2 min-h-[150px]"
                    >
                      <div className="flex items-center gap-2 px-2 pb-3 pt-1 border-b border-border-subtle/40 mb-3">
                        ...
                        <span className="text-xs font-mono bg-border-subtle text-text-muted px-1.5 py-0.5 rounded ml-auto">
                          {filtered.length}
                        </span>
                      </div>
                      <div className="flex flex-col gap-2 flex-1">
                            ...
                            >
                              <div className="flex items-start justify-between gap-2 mb-2">
                                <div className="flex flex-col min-w-0">
                                  <span className="text-xs font-semibold text-text-main line-clamp-2 leading-snug">
                                    {mat.title}
                                  </span>
                                  {mat.description && (
                                    <span className="text-[11px] text-text-muted mt-1 line-clamp-2">
                                      {mat.description}
                                    </span>
                                  )}
                                </div>
                                {mat.is_required && !mat.viewed && (
                                  <span
                                    className="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0 mt-1"
                                    title="Обязательно"
                                  />
                                )}
                              </div>
                              <div className="flex items-center justify-between gap-2 mt-auto pt-2 border-t border-border-subtle/30">
                                ...
                                {mat.viewed ? (
                                  ...
                                  <Button
                                    ...
                                    variant="flat"
                                    color="primary"
                                    className="h-6 text-[10px] font-mono px-2 min-w-0"
                                    onClick={() => markViewed(mat.id)}
                                  >
                                    Изучено
                                  </Button>
                                )}
                        ...
                        {filtered.length === 0 && (
                          <div className="text-center text-text-muted text-[11px] p-4">
                            Нет материалов
                          </div>
                        )}
                      </div>
