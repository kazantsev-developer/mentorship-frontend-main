...
import { Accordion, AccordionItem, Button, Chip, Spinner, Link } from '@heroui/react';
...
export default function StudentRoadmap() {
  ...
  const getStatusInfo = (status: string) => {
    switch(status) {
      case 'in_progress': return { color: 'primary', text: 'В процессе' };
      case 'waiting_buddy_confirmation': return { color: 'warning', text: 'Ожидает Buddy' };
      case 'approved': return { color: 'success', text: 'Подтверждён' };
      default: return { color: 'default', text: 'Не начат' };
    }
  };
    ...
    <div className="w-full max-w-[1400px] mx-auto bg-canvas min-h-screen text-text-main py-4">
      <Accordion variant="splitted" className="px-0 gap-4">
        {blocks.map((block) => {
          ...
          const percent = block.ProgressPercent || 0;
          return (
            <AccordionItem
              key={block.ID}
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 py-4 border-t border-border-subtle mt-2">
                {COLUMNS.map((col) => {
                  const filtered = materials.filter((m: any) => m.Type === col.type);
                  return (
                          ...
                          {filtered.map((mat: any, idx: number) => (
                            ...
                            >
                              <div className="flex items-start justify-between gap-2 mb-2">
                                ...
                                {mat.IsRequired && !mat.Viewed && <span className="w-1.5 h-1.5 rounded-full bg-rose-500 shrink-0 mt-1" title="Обязательно" />}
                              </div>
